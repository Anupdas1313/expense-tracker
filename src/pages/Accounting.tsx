  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-[#222222]">Accounting & Period Close</h1>
          <p className="text-[#717171] mt-1">Analyze monthly transactions and lock accounting periods.</p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {displayMonths.map(monthStr => {
          ...
          return (
            <div key={monthStr} className={`bg-white rounded-[24px] shadow-[0_6px_16px_rgba(0,0,0,0.04)] border overflow-hidden transition-colors ${data.isClosed ? 'border-[#EBEBEB]' : 'border-neutral-200'}`}>
              <div className={`p-5 border-b flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 ${data.isClosed ? 'bg-neutral-50 border-[#EBEBEB]' : 'bg-white border-neutral-200'}`}>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${data.isClosed ? 'bg-neutral-200 text-[#717171]' : 'bg-neutral-100 text-[#222222]'}`}>
                    {data.isClosed ? <Lock className="w-5 h-5" /> : <Unlock className="w-5 h-5" />}
                  </div>
                  <div>
                    <h2 className="text-lg font-bold text-[#222222]">{format(monthDate, 'MMMM yyyy')}</h2>
                ...
                <div className="w-full sm:w-auto">
                  {data.isClosed ? (
                    <button
                      ...
                      className="w-full sm:w-auto px-4 py-2 text-sm font-bold text-[#222222] bg-white border border-[#EBEBEB] rounded-xl hover:bg-neutral-50 transition-colors"
                    >
                      Reopen Month
                    </button>
                  ) : (
                    <button
                      ...
                      className="w-full sm:w-auto px-4 py-2 text-sm font-bold text-white bg-[#222222] rounded-xl hover:bg-black transition-colors"
                    >
                      Close Month
                    </button>
                  )}
              ...
              <div className="p-6">
                {isDiscrepancy && (
                  <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3 text-amber-800">
                    ...
                    <div>
                      <p className="font-bold">Balance Discrepancy Detected</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <div>
                    <p className="text-sm font-medium text-[#717171] mb-1">Opening Balance</p>
                  ...
                  <div>
                    <p className="text-sm font-medium text-[#717171] mb-1 flex items-center gap-1">
                  ...
                  <div className="pt-4 md:pt-0 md:border-l md:border-[#EBEBEB] md:pl-6">
                    <p className="text-sm font-medium text-[#717171] mb-1 flex items-center gap-1">
                ...
                {data.isClosed && (
                  <div className="mt-6 pt-6 border-t border-[#EBEBEB]">
                    <h3 className="text-sm font-bold text-[#222222] mb-3">Account Balances at Close</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      {Object.entries(data.accountBalances).map(([accountId, balance]) => {
                        ...
                        return (
                          <div key={accountId} className="flex justify-between items-center p-3 bg-neutral-50 rounded-xl border border-[#EBEBEB]">
                            <div className="flex items-center gap-2">
                              <div className="w-6 h-6 bg-white rounded-full flex items-center justify-center text-xs font-bold text-[#717171] border border-[#EBEBEB] overflow-hidden p-0.5">
                              ...
                              <span className="text-sm font-bold text-[#222222]">{account.bankName}</span>
                            </div>
        ...
        {displayMonths.length === 0 && (
          <div className="text-center py-12 bg-white rounded-[24px] border border-[#EBEBEB]">
            <Wallet className="w-12 h-12 text-[#B0B0B0] mx-auto mb-3" />
            <h3 className="text-lg font-bold text-[#222222]">No accounting periods yet</h3>
            <p className="text-[#717171] mt-1">Add some transactions to see your monthly analysis.</p>
          </div>
        )}
      </div>
    </div>
